...
class StartEndDataset_audio(Dataset):
    ...
    def __init__(self, dset_name, data_path, v_feat_dirs, q_feat_dirs, a_feat_dir=None,
    ...
    #     if self.dset_name == 'tvsum':
    #         q_feat = np.load(join(self.q_feat_dirs, "{}.npz".format(qid))) # 'token', 'text'
    #         return torch.from_numpy(q_feat['token'])
    #     else:
    #         # QVhighlight dataset
    #         q_feat_path = join(self.q_feat_dirs, f"qid{qid}.npz")
